...
def get_max_duration(año, plataforma, tipo):
    ...
    id = df_plataformas.reset_index(drop=True).query(f"release_year == {año} and platform == '{plataforma.capitalize()}' and unit == '{tipo.capitalize()}'").duration.idxmax()

    titulo = df_plataformas.reset_index(drop=True).title[id]
    duracion = df_plataformas.reset_index(drop=True).duration[id]
...
def get_count_platform(plataforma):
    ...
    tvShow_Movies = df_plataformas.query(f"platform == '{plataforma.capitalize()}'").groupby(['type', 'platform']).count()
...
def get_listedin(genero):
    ...
    cantidad_por_plataforma = df_plataformas[df_plataformas.listed_in.str.contains(genero.title())].groupby(['platform']).title.count().sort_values(ascending=False)
...
def get_actor(plataforma, año):
    ...
    df_func = df_plataformas.query(f"platform == '{plataforma.capitalize()}' and release_year == {año}")
